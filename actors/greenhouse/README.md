...
```json
{
  "boards": ["stripe.com", "https://job-boards.greenhouse.io/anthropic"],
...
The board token is **the company slug in job-boards.greenhouse.io/<company>** — but you rarely need to look it up.
...
That is Greenhouse's own public board API - the one that fills its customers'
...
the identifier itself - `stripe.com` is a working example - and this Actor
resolves it. If you already know the token - the company slug in job-boards.greenhouse.io/<company> - that works too.
...
Either it is not on Greenhouse - most large enterprises use Workday, Taleo, iCIMS
...
Greenhouse's endpoint above is public and documented, and so are the other five.
