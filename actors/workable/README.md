...
Workable exposes department, employment type and a remote flag, so `department`, `employmentType` and `isRemote` are usually populated.
...
The board token is **the company slug in apply.workable.com/<company>** — but you rarely need to look it up.
...
That is Workable's own public board API - the one that fills its customers'
...
the identifier itself - `huggingface.co` is a working example - and this Actor
resolves it. If you already know the token - the company slug in apply.workable.com/<company> - that works too.
...
Either it is not on Workable - most large enterprises use Workday, Taleo, iCIMS
...
Workable's endpoint above is public and documented, and so are the other five.
