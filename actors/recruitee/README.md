...
Give it a list of Recruitee board tokens. It calls the official public Recruitee
board API for each one and returns every live opening in a flat,
spreadsheet-ready schema.

This reads the API Recruitee already publishes for its customers' own careers
...
Recruitee exposes department and a structured employment type code, so `department` and `employmentType` are usually populated.
...
```json
{
  "boards": ["ibexa.co", "https://enseradesign.recruitee.com"],
...
The board token is **the subdomain in <company>.recruitee.com** — but you rarely need to look it up.
...
- Recruitee boards only. For a company on a different platform, see the
...
That is Recruitee's own public board API - the one that fills its customers'
...
**Does Recruitee have a public jobs API?**
...
the identifier itself - `ibexa.co` is a working example - and this Actor
resolves it. If you already know the token - the subdomain in <company>.recruitee.com - that works too.
...
Either it is not on Recruitee - most large enterprises use Workday, Taleo, iCIMS
...
Recruitee's endpoint above is public and documented, and so are the other five.
