...
Reads the official public Recruitee board API. One Actor, one platform, no
...
MISS_HINT = (
    "No Recruitee board answered for this entry. The company may be on a platform "
    ...
    "sits on is enough. The token is the subdomain in <company>.recruitee.com."
)
...
async def main() -> None:
    async with Actor:
        ...
        Actor.log.info("%d Recruitee board(s), concurrency %d" % (len(boards), concurrency))
        ...
        async with httpx.AsyncClient(
            ...
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; RecruiteeJobsScraper/1.0)",
            ...
            async def handle(entry) -> None:
                ...
                if error:
                    ...
                    await Actor.push_data({
                        "recordType": "notFound",
                        "companyName": token,
                        "title": "Not found on Recruitee - see the hint column",
                        "boardToken": token,
                        "ats": ATS,
                        "error": error,
                        "hint": MISS_HINT,
        ...
        if not delta.enabled:
            summary = ("Done. %d jobs from %d Recruitee board(s) (%d not found)."
                       % (totals["jobs"], totals["ok"], totals["failed"]))
        elif delta.is_baseline:
            summary = ("Baseline recorded: %d jobs from %d Recruitee board(s). "
                       "The next run returns only changes."
                       % (totals["jobs"], totals["ok"]))
        else:
            summary = ("%d new, %d closed, across %d Recruitee board(s)."
