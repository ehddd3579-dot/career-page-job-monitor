...
Reads the official public Ashby board API. One Actor, one platform, no
...
ATS = "ashby"
_TOKEN_FROM_URL = re.compile(r"jobs\.ashbyhq\.com/([\w.-]+)", re.I)
...
MISS_HINT = (
    "No Ashby board answered for this entry. The company may be on a platform "
    ...
    "sits on is enough. The token is the company slug in jobs.ashbyhq.com/<company>."
)
...
async def main() -> None:
    async with Actor:
        ...
        if not boards:
            raise ValueError(
                "Input 'boards' is empty. Add Ashby board tokens - the company slug in jobs.ashbyhq.com/<company>."
            )
        ...
        Actor.log.info("%d Ashby board(s), concurrency %d" % (len(boards), concurrency))
        ...
        async with httpx.AsyncClient(
            ...
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; AshbyJobsScraper/1.0)",
            ...
            async def handle(entry) -> None:
                ...
                if error:
                    ...
                    await Actor.push_data({
                        "recordType": "notFound",
                        "companyName": token,
                        "title": "Not found on Ashby - see the hint column",
                        "boardToken": token,
                        "ats": ATS,
                        "error": error,
                        "hint": MISS_HINT,
        ...
        if not delta.enabled:
            summary = ("Done. %d jobs from %d Ashby board(s) (%d not found)."
                       % (totals["jobs"], totals["ok"], totals["failed"]))
        elif delta.is_baseline:
            summary = ("Baseline recorded: %d jobs from %d Ashby board(s). "
                       "The next run returns only changes."
                       % (totals["jobs"], totals["ok"]))
        else:
            summary = ("%d new, %d closed, across %d Ashby board(s)."
