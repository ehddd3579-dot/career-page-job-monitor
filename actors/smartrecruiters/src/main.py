...
MISS_HINT = (
    "No SmartRecruiters board answered for this entry. The company may be on a platform "
    ...
    "sits on is enough. The token is the identifier in careers.smartrecruiters.com/<Company> (case sensitive)."
)
            ...
            async def handle(entry) -> None:
                ...
                if error:
                    ...
                    await Actor.push_data({
                        "recordType": "notFound",
                        "companyName": token,
                        "title": "Not found on SmartRecruiters - see the hint column",
                        "boardToken": token,
                        "ats": ATS,
                        "error": error,
                        "hint": MISS_HINT,
