...
Give it a list of Lever board tokens. It calls the official public Lever
board API for each one and returns every live opening in a flat,
spreadsheet-ready schema.

This reads the API Lever already publishes for its customers' own careers
...
Lever exposes team, commitment and workplace type, so `team`, `employmentType` and `workplaceType` are usually populated.
...
The board token is **the company slug in jobs.lever.co/<company>** — but you rarely need to look it up.
...
- Lever boards only. For a company on a different platform, see the
...
That is Lever's own public board API - the one that fills its customers'
...
**Does Lever have a public jobs API?**
...
the identifier itself - `leverdemo` is a working example - and this Actor
resolves it. If you already know the token - the company slug in jobs.lever.co/<company> - that works too.
...
Either it is not on Lever - most large enterprises use Workday, Taleo, iCIMS
...
Lever's endpoint above is public and documented, and so are the other five.
